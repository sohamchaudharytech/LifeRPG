...
import { Shield, User, Mail, KeyRound, CheckCircle2, ArrowLeft, RefreshCw } from "lucide-react";

export default function RegisterPage() {
  const router = useRouter();
  const [step, setStep] = useState<1 | 2>(1); // 1: Info, 2: 8-digit OTP
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/otp/send", {
        ...
        body: JSON.stringify({ username, email, type: "registration" }),
      });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setSuccessMessage("A fresh 8-digit code has been dispatched.");
      setResendCooldown(45);
    } catch (err: any) {
      setError(err.message);
    } finally {
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black font-sans tracking-tight text-white mb-2">
            {step === 1 ? "FORGE YOUR HERO" : "ENTER SECURE PASSKEY"}
          </h1>
        ...
        <Card className="p-8 bg-surface-elevated/95 border-white/10 shadow-2xl">
          ...
          {step === 1 ? (
            <form onSubmit={handleSendOtp} className="space-y-4">
              ...
              >
                REQUEST 8-DIGIT VERIFICATION CODE
              </Button>
            </form>
          ) : (
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full text-center tracking-[0.35em] text-xl font-mono py-3 rounded-xl bg-surface border border-violet-500/40 text-white placeholder-slate-600 focus:outline-none focus:border-cyan-400"
                  />
                </div>
                <p className="text-[11px] text-slate-400 font-mono mt-1.5 text-center">
                  Sent to <span className="text-slate-200">{email}</span> (Expires in 10m)
                </p>
              </div>
